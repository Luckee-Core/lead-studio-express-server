...
import {
  buildLinkedInGoogleSearchQuery,
...
import type { LinkedinSerpDiscoveryInput, LinkedinSerpDiscoveryResult } from '../types';

/**
 * Discovery phase for LinkedIn profile search:
 * 1) Load lead context
 * 2) Build best-available query (name/location + optional website)
 * 3) Scrape SERP candidate links
 */
export const runLinkedinSerpDiscovery = async (
  input: LinkedinSerpDiscoveryInput
): Promise<LinkedinSerpDiscoveryResult> => {
  ...
  const searchBusinessName = primaryWebsite
    ? lead.business_name
    : `${lead.business_name} linkedin company`;
