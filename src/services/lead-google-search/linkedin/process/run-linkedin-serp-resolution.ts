...
import type { LinkedinSerpResolutionInput, LinkedinSerpResolutionResult } from '../types';

/**
 * Resolution phase for LinkedIn profile search:
 * use AI to pick the best candidate URL and persist AI audit records.
 */
export const runLinkedinSerpResolution = async (
  input: LinkedinSerpResolutionInput
): Promise<LinkedinSerpResolutionResult> => {
  const {
    supabase,
    lead,
    runId,
  ...
  if (serpLinks.length > 0 && anthropic) {
    console.log('🤖 lead-google-search/linkedin: resolving single-platform SERP', {
      leadId: lead.id,
    ...
    if (ai.success && ai.url) {
      ...
      console.warn('⚠️ lead-google-search/linkedin: AI resolution failed', aiError);
    } else {
      console.log('ℹ️ lead-google-search/linkedin: no matching URL for platform', { leadId: lead.id });
    }
  } else if (serpLinks.length > 0 && !anthropic) {
    aiError = 'ANTHROPIC_API_KEY not configured';
    console.warn('⚠️ lead-google-search/linkedin: skipping AI —', aiError);
  }
