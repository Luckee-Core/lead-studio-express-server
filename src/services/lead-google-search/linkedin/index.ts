...
import { runLinkedinSerpDiscovery, runLinkedinSerpResolution } from './process';

/**
 * POST /api/services/lead-google-search/linkedin — manual only.
 *
 * Body must include `leadId`.
 */
export const runLeadGoogleSearchLinkedin = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    try {
      // Phase 1: Discovery. Build lead context and scrape candidate LinkedIn URLs from Google.
      const discovery = await runLinkedinSerpDiscovery({ supabase, leadId });
      // Phase 2: Resolution. Use AI to choose the best match from discovery candidates.
      const resolution = await runLinkedinSerpResolution({
        supabase,
        runId,
        ...discovery,
      });
      ...
      if (resolution.resolvedUrl) {
        await updateLead(supabase, discovery.lead.id, { linkedin_url: resolution.resolvedUrl });
        leadUpdated = true;
        console.log('💾 lead-google-search/linkedin: lead URL updated', { leadId: discovery.lead.id });
      }
    ...
    console.error('❌ runLeadGoogleSearchLinkedin:', err);
